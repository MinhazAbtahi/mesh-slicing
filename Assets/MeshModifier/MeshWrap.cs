...
[System.Serializable]
public class MeshBindVertex
{
	public float				weight;
	public List<MeshBindInfo>	verts = new List<MeshBindInfo>();
}

public struct MeshCloseFace
{
	public int face;
	public float dist;
}
...
public class MeshWrap : MonoBehaviour
{
	...
	public MeshBindVertex[]	bindverts;
	public MeshModifyObject	target;
	public float			maxdist			= 0.25f;
	public int				maxpoints		= 4;
	public bool				WrapEnabled		= true;
	public NormalMethod NormalMethod	= NormalMethod.Unity;
	...
	{
		return NearestPoint.DistPoint3Triangle3Dbl(p, p0, p1, p2);
	}
	...
	public void Attach(MeshModifyObject modobj)
	{
		targetIsSkin = false;
		sourceIsSkin = false;
		...
		bindverts = new MeshBindVertex[verts.Length];
		...
		List<MeshCloseFace> closefaces = new List<MeshCloseFace>();
		...
		{
			MeshBindVertex bv = new MeshBindVertex();
			bindverts[i] = bv;
			...
			for ( int t = 0; t < basefaces.Length; t += 3 )
			{
				...
				{
					MeshCloseFace cf = new MeshCloseFace();
			...
			for ( int j = 0; j < maxp; j++ )
			{
				if ( j < closefaces.Count )
				{
					...
					MeshBindInfo bi = new MeshBindInfo();
		...
		if ( targetIsSkin && !sourceIsSkin )
		{
			...
			for ( int i = 0; i < bindverts.Length; i++ )
			{
				if ( bindverts[i].verts.Count > 0 )
				{
					...
					for ( int j = 0; j < cnt; j++ )
					{
						MeshBindInfo bi = bindverts[i].verts[j];
	...
	{
		if ( NormalMethod == NormalMethod.Unity )	//|| mapping == null )
			mesh.RecalculateNormals();
		else
		{
			if ( mapping == null )
