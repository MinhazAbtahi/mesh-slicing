...
{
	public MeshBoundingBox			bbox;
	public Vector3			Offset;
	public MeshModifiers	mod;
...
public class MeshModifiers : MonoBehaviour
{
	[HideInInspector]
	...
	Vector3[]				tan1;
	Vector3[]				tan2;
	public ModContext	modContext = new ModContext();
	...
	private static int CompareOrder(MeshModifier m1, MeshModifier m2)
	{
	...
	{
		if ( NormalMethod == NormalMethod.Unity || dynamicMesh )	//|| mapping == null )
...
//#endif
		...
		if ( Enabled && mods != null )
		{
			...
			if ( Cores > 0 )
			{
				...
				{
					tasks[i].jobtype	= JobType.Modifier;
					tasks[i].index		= i + 1;
	...
	void DoWork(object info)
	{
		...
		while ( isRunning )
		{
			...
			switch ( inf.jobtype )
			{
				case JobType.Modifier:
					if ( inf.end > 0 )
					{
						for ( int m = 0; m < inf.modcontext.mod.mods.Length; m++ )
						{
							MeshModifier mod = inf.modcontext.mod.mods[m];
	...
	void OnApplicationQuit()
	{
		...
		{
			MeshModifiers[] modsrunning = (MeshModifiers[])Resources.FindObjectsOfTypeAll(typeof(MeshModifiers));
	...
	public CustomNormalMap[]	mapping;
	public int[]		tris;
	...
	public void BuildNormalMapping(Mesh mesh, bool force)
	{
		if ( mapping == null || mapping.Length == 0 || force )
		{
			...
			mapping = new CustomNormalMap[verts.Length];

			for ( int i = 0; i < verts.Length; i++ )
			{
				mapping[i] = new CustomNormalMap();
	...
	public void RecalcFaceNormalsMT(MeshModifiers mod, int cnum, int cores)
	{
		Vector3 v30 = Vector3.zero;
		Vector3 v31 = Vector3.zero;
		Vector3 v32 = Vector3.zero;
	...
	public void RecalcVertexNormalsMT(MeshModifiers mod, int cnum, int cores)
	{
		Vector3 v30 = Vector3.zero;
	...
	public void RecalcNormalsMT(Mesh ms, Vector3[] _verts)
	{
		...
		for ( int i = 0; i < Cores; i++ )
		{
			tasks[i].jobtype = JobType.FaceNormalCalc;
			tasks[i].end = 1;
			tasks[i].pauseevent.Set();
		}
		...
		for ( int i = 0; i < Cores; i++ )
		{
			tasks[i].jobtype = JobType.VertexNormalCalc;
			tasks[i].end = 1;
			tasks[i].pauseevent.Set();
		}
	...
	void BuildTangentsMT(Mesh ms, Vector3[] _verts)
	{
		...
		for ( int i = 0; i < Cores; i++ )
		{
			tasks[i].jobtype = JobType.FaceTangentCalc;
			tasks[i].end = 1;
			tasks[i].pauseevent.Set();
		}
		...
		for ( int i = 0; i < Cores; i++ )
		{
			tasks[i].jobtype = JobType.VertexTangentCalc;
			tasks[i].end = 1;
			tasks[i].pauseevent.Set();
		}
	...
	void BuildFaceTangentsMT(MeshModifiers mc, int cnum, int cores)
	{
	...
	void BuildVertexTangentsMT(MeshModifiers mc, int cnum, int cores)
	{
