...
public class MeshCopyObject
{
	...
	static GameObject CopyMesh(GameObject subject, MeshModifyObject mod)
	{
		...
		for ( int i = 0; i < l; i++ )
		{
			...
			if ( mod.NormalMethod == NormalMethod.Custom && mod.norms != null && mod.norms.Length > 0 )
	...
	static void CopyModObj(MeshModifyObject from, MeshModifyObject to)
	{
		if ( from && to )
		{
	...
	{
		MeshModifyObject fromMod = from.GetComponent<MeshModifyObject>();
		...
		if ( fromMod )
			...
			to = CopyMesh(from);
		MeshModifyObject mo = to.AddComponent<MeshModifyObject>();

		CopyModObj(fromMod, mo);

		MeshModifier[] mods = from.GetComponents<MeshModifier>();
		...
		for ( int i = 0; i < mods.Length; i++ )
		{
			...
			if ( com )
			{
				MeshModifier mod = (MeshModifier)com;
				mod.PostCopy(mods[i]);
			}
		}

		MeshWrap wrap = from.GetComponent<MeshWrap>();
	...
	static public GameObject DeepCopy(GameObject subject)
	{
		...
		if ( subject )
		{
			...
			MeshModifyObject[] modobjs = clone.GetComponentsInChildren<MeshModifyObject>();
	...
	public static Mesh DupMesh(Mesh mesh, MeshModifyObject mod)
	{
		Mesh clonemesh = new Mesh();
		clonemesh.vertices = mod.verts;	//mesh.vertices;
#if UNITY_5_0 || UNITY_5_1 || UNITY_5 || UNITY_2017 || UNITY_2018
    ...
    static public GameObject DuplicateObject(GameObject from)
    {
        ...
        if (from)
        {
            ...
            if (newobj)
            {
                ...
                MeshModifyObject[] mobjs = newobj.GetComponentsInChildren<MeshModifyObject>();

                for (int i = 0; i < mobjs.Length; i++)
                    mobjs[i].MeshUpdated();

                MeshModifier[] frommods = from.GetComponentsInChildren<MeshModifier>();
                MeshModifier[] tomods = newobj.GetComponentsInChildren<MeshModifier>();

                for (int i = 0; i < frommods.Length; i++)
                    tomods[i].PostCopy(frommods[i]);

                MeshWrap[] wraps = newobj.GetComponentsInChildren<MeshWrap>();
    ...
    static public GameObject DuplicateObjectForPrefab(GameObject from)
    {
        ...
        if (from)
        {
            ...
            if (newobj)
            {
                ...
                for (int i = 0; i < mobjs.Length; i++)
                {
                    Mesh m = MeshModifyObject.FindMesh(mobjs[i].gameObject);
                    if (m)
                    {
                        Mesh newmesh = DupMesh(m, mobjs[i]);
                        MeshModifyObject.SetMeshNew(mobjs[i].gameObject, newmesh);
                    }
                }
