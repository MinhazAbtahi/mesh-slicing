...
public class MeshModifyObject : MeshModifiers
{
	...
	{
		MeshModifier[] mods = GetComponents<MeshModifier>();
	...
	void OnRenderObject()
	{
		if ( UpdateMode == UpdateMode.OnRender )
		{
			ModifyObjectMT();
		}
	}
	...
	public void GetMesh(bool force)
	{
		if ( mesh == null || cachedMesh == null || sverts.Length == 0 || mesh.vertexCount != sverts.Length || force )
		{
			...
			else
			{
				...
				if ( verts != null && verts.Length > 0 )
					cachedMesh.vertices = verts;
				mesh = MeshCopyObject.DupMesh(cachedMesh, "");
	...
	public void ModReset(MeshModifier m)
	{
		if ( m != null )
		{
	...
	public void MeshUpdated()
	{
		...
		foreach ( MeshModifier mod in mods )	// Added back in?
	...
	public void MeshChanged(Mesh newmesh)
	{
		if ( mesh )
		{
			...
			foreach ( MeshModifier mod in mods )
			{
				if ( mod != null )
				{
